import urllib.request
import urllib.parse
import json
import uuid
import datetime
import mimetypes
import os
from io import BytesIO

# 1. Login
TEST_NISN = os.environ.get("MODIVA_TEST_NISN", "10001")
TEST_SCHOOL_CODE = os.environ.get("MODIVA_TEST_SCHOOL_CODE", "20223819")

data = json.dumps({"nisn": TEST_NISN, "schoolCode": TEST_SCHOOL_CODE}).encode("utf-8")
req_login = urllib.request.Request("http://127.0.0.1:8000/api/auth/login-siswa", data=data, headers={"Content-Type": "application/json"})
login_res = urllib.request.urlopen(req_login)
login_data = json.loads(login_res.read().decode("utf-8"))
token = login_data["token"]
print("Login successful, token:", token)

# 2. Submit Report
boundary = uuid.uuid4().hex
headers = {
    "Authorization": f"Bearer {token}",
    "Content-Type": f"multipart/form-data; boundary={boundary}"
}

def create_multipart_formdata(fields, files):
    body = BytesIO()
    for key, value in fields.items():
        body.write(f"--{boundary}\r\n".encode('utf-8'))
        body.write(f'Content-Disposition: form-data; name="{key}"\r\n\r\n'.encode('utf-8'))
        body.write(f"{value}\r\n".encode('utf-8'))
    for key, (filename, content, mime) in files.items():
        body.write(f"--{boundary}\r\n".encode('utf-8'))
        body.write(f'Content-Disposition: form-data; name="{key}"; filename="{filename}"\r\n'.encode('utf-8'))
        body.write(f'Content-Type: {mime}\r\n\r\n'.encode('utf-8'))
        body.write(content)
        body.write(b'\r\n')
    body.write(f"--{boundary}--\r\n".encode('utf-8'))
    return body.getvalue()

fields = {
    "date": datetime.datetime.now().strftime("%Y-%m-%d"),
    "notes": "Test report from automated script"
}
# Create a dummy 1x1 jpeg image
dummy_image = b'\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00H\x00H\x00\x00\xff\xdb\x00C\x00\x08\x06\x06\x07\x06\x05\x08\x07\x07\x07\t\t\x08\n\x0c\x14\r\x0c\x0b\x0b\x0c\x19\x12\x13\x0f\x14\x1d\x1a\x1f\x1e\x1d\x1a\x1c\x1c $.\' ",#\x1c\x1c(7),01444\x1f\'9=82<.342\xff\xdb\x00C\x01\t\t\t\x0c\x0b\x0c\x18\r\r\x182!\x1c!22222222222222222222222222222222222222222222222222\xff\xc0\x00\x0b\x08\x00\x01\x00\x01\x03\x01"\x00\x02\x11\x01\x03\x11\x01\xff\xc4\x00\x1f\x00\x00\x01\x05\x01\x01\x01\x01\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x01\x02\x03\x04\x05\x06\x07\x08\t\n\x0b\xff\xc4\x00\xb5\x10\x00\x02\x01\x03\x03\x02\x04\x03\x05\x05\x04\x04\x00\x00\x01}\x01\x02\x03\x00\x04\x11\x05\x12!1A\x06\x13Qa\x07"q\x142\x81\x91\xa1\x08#B\xb1\xc1\x15R\xd1\xf0$3br\x82\t\n\x16\x17\x18\x19\x1a%&\'()*456789:CDEFGHIJSTUVWXYZcdefghijstuvwxyz\t\n\x0b\xff\xc4\x00\xb5\x11\x00\x02\x01\x02\x04\x04\x03\x04\x07\x05\x04\x04\x00\x01\x02\x11\x00\x01\x02\x03\x11\x04\x05!1\x06\x12AQ\x07aq\x13"2\x81\x08\x14B\x91\xa1\xb1\xc1\t#3R\xf0\x15br\xd1\n\x16$4\xe1%\xf1\x17\x18\x19\x1a&\'()*56789:CDEFGHIJSTUVWXYZcdefghijstuvwxyz?\x00\xfd\xfc\xa8\xff\xd9'
files = {"photo": ("test_photo.jpg", dummy_image, "image/jpeg")}
body = create_multipart_formdata(fields, files)

req_submit = urllib.request.Request("http://127.0.0.1:8000/api/reports/submit", data=body, headers=headers)
try:
    submit_res = urllib.request.urlopen(req_submit)
    print("Submit status:", submit_res.status)
    print("Submit body:", submit_res.read().decode("utf-8"))
except urllib.error.HTTPError as e:
    print("Submit error:", e.code, e.read().decode("utf-8"))

# 3. Verify Database Table
req_verify = urllib.request.Request("http://127.0.0.1:8000/api/reports?page=1&limit=5", headers={"Authorization": f"Bearer {token}"})
try:
    verify_res = urllib.request.urlopen(req_verify)
    verify_data = json.loads(verify_res.read().decode("utf-8"))
    reports = verify_data["data"]["reports"]
    if len(reports) > 0:
        print(f"Verification Success! Found {len(reports)} reports. Latest report id: {reports[0]['id']}")
    else:
        print("Verification Failed! No reports found.")
except Exception as e:
    print("Verify error:", e)
